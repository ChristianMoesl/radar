...
The daemon automatically garbage-collects local workspaces for tasks that have been done for at least 24 hours. A registered multi-worktree workspace is one conservative cleanup bundle: Radar skips all of it if any member is dirty, any deletable managed branch has commits not found on a remote-tracking ref, branch publication cannot be checked, or its shared tmux session is attached. Run `radar gc`, or press `X` in the TUI, to clean eligible done-task workspaces immediately without waiting for the 24-hour retention period. Manual garbage collection keeps the same safety checks.
...
Cleanup issues are visible before running GC. The overview shows **⚠️ unresolved** when a linked local resource has a cleanup-preventing issue. Press `i` to see a separate **Unresolved** section at the top of Inspect, identifying each affected resource and its exact reason. The section is hidden when there are no issues. These checks cover uncommitted changes, branch publication and verification failures, unrecognised workspace-root files, attached sessions, out-of-root workspaces, and provider inspection errors. Being active or within the GC retention period is not an unresolved issue. Done tasks with a current unresolved workspace remain visible beyond the usual three-day display limit, so their Inspect details stay accessible. Once the issues clear or the workspace is removed, normal display retention applies again; task completion dates and GC eligibility are unchanged.

Local collection reuses cleanup preview checks to populate resource issue snapshots. It refreshes local checks each collection; remote fetch outcomes for these observations are shared per repository for up to two minutes, with bounded checks and concurrency. Cleanup itself still performs fresh checks before removal. Rendering Inspect and changing authored task state do not run safety checks or fetch remotes. Issue snapshots are optional fields in the existing rebuildable state cache; existing cache files remain readable and are populated on the next local collection without migration or reset.
...
Inspect is a read-only view of the selected task. When cleanup issues exist, its first section is **Unresolved**, with affected resources and reasons; otherwise this section is omitted. Use `j`/`k`, arrow keys, or Ctrl+N/P to scroll; Page Up/Down or Ctrl+U/D to page; and `g`/`G` or Home/End to jump to the top/bottom. Long paths and URLs wrap, while the task title, line position, and shortcuts stay visible. Scrolling never selects another task or switches workspaces. Live updates retain the inspected task; if it disappears, inspect shows “Task no longer available” rather than another task. Escape or Backspace returns to the overview, where Enter and task/workspace actions remain available. `q` or Ctrl+C quits.
