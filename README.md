...
After `X` completes, the TUI opens a read-only results view with deleted/skipped workspace counts, every skipped workspace's path and task ID, and its reported skip reason. Skipped workspaces appear first; full paths and reasons wrap without truncation. Use ↑/↓ or PgUp/PgDn to scroll, Home/End to jump, and Enter/Esc to return. The results stay visible during background task refreshes. `radar gc` returns the same paths and reasons as JSON. Radar also sends the garbage-collection summary as a host OS notification.
