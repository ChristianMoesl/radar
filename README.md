...
The TUI summarizes local resources on each task row with [Nerd Font](https://www.nerdfonts.com/) icons and counts: ` 2` for Git worktrees, ` 1` for Docker SBX sandboxes, and ` 1` for tmux sessions. Configure your terminal to use a current Nerd Font. Zero counts are omitted. A ` dirty` warning appears if any linked worktree has uncommitted changes, including members of a Radar workspace. Counts and the warning remain visible when long task text is truncated.
