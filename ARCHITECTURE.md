...
- `SourceRef.ProvidesWorkspace`: declares that an authoritative ref owns the persistent local working directory in its absolute `Path`. The ref also emits the matching cleaned `workspace:<path>` linking key. This capability is independent of lifecycle ownership: Obsidian work items and Git workspace refs provide workspaces, while tmux and SBX resources only consume workspace paths.
