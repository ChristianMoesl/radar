...
func githubRef(id string, repo string, branch string) protocol.SourceRef {
	return protocol.SourceRef{ID: id, Source: "github", Kind: "pull_request", Role: protocol.SourceRefRoleAuthoritative, Repo: repo, Branch: branch, CanonicalKey: id, LinkingKeys: linking.Keys("ticket:CAP-7", linking.BranchKey(repo, branch))}
}

func worktreeRef(path string, repo string, branch string) protocol.SourceRef {
	return protocol.SourceRef{ID: "git:worktree:" + path, Source: "git", Kind: "worktree", Role: protocol.SourceRefRoleAuthoritative, Repo: repo, Path: path, Branch: branch, CanonicalKey: linking.WorkspaceKey(path), LinkingKeys: linking.Keys("ticket:CAP-7", linking.WorkspaceKey(path), linking.BranchKey(repo, branch))}
}

func detachedSessionRef(path string, name string) protocol.SourceRef {
	return protocol.SourceRef{ID: "tmux:session:" + name, Source: "tmux", Kind: "session", Role: protocol.SourceRefRoleAuthoritative, Title: name, Path: path, Status: "detached", LinkingKeys: linking.Keys("ticket:CAP-7", linking.WorkspaceKey(path)), Metadata: map[string]string{"session": name, "attached_count": "0"}}
}
...
func sandboxRef(path string, name string) protocol.SourceRef {
	return protocol.SourceRef{ID: "sbx:sandbox:" + name, Source: "sbx", Kind: "sandbox", Role: protocol.SourceRefRoleAuthoritative, Title: name, Path: path, LinkingKeys: linking.Keys("ticket:CAP-7", linking.WorkspaceKey(path)), Metadata: map[string]string{"name": name}}
}
